# ------------------------------------------------------------
# 【中文导读】本脚本估计多状态转移概率（5/10/15年）及标准误。
# 步骤1：构建不同转移场景的数据（Baseline->CMD/CA/Death等）。
# 步骤2：统一为mstate可用的long格式，并定义转移矩阵。
# 步骤3：扩展协变量并拟合分层Cox模型。
# 步骤4：使用均值协变量构造newdata，调用msfit/probtrans计算转移概率。
# 步骤5：汇总各阶段结果到all_trans.csv。
# ------------------------------------------------------------
# ============================================================
