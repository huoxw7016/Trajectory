# ------------------------------------------------------------
# 【中文导读】本脚本在多状态框架下筛选多组学标志物（重点是蛋白），并组织训练/测试分析流程。
# 步骤1：读入轨迹、协变量、蛋白数据并合并。
# 步骤2：对蛋白矩阵做标准化，减少量纲差异影响。
# 步骤3：按birthplace等规则划分训练/测试集。
# 步骤4：定义协变量与候选蛋白集合，逐转移拟合模型并导出显著标志物。
# 步骤5：结果用于后续富集分析、PCA和预测模型。
# ------------------------------------------------------------
# ============================================================
