# ------------------------------------------------------------
# 【中文导读】本脚本在原始ICD10编码中逐步识别疾病事件，并计算首次发生时间（相对基线年）。
# 步骤1：先定义非癌疾病（T2D/CVD/CAD/HF/Stroke）的编码匹配规则。
# 步骤2：对每个受试者逐列扫描住院诊断编码（s_41270_0_*）。
# 步骤3：一旦匹配到目标编码，记录事件=1，并收集对应日期（s_41280_0_*）。
# 步骤4：对同一疾病的多次记录取最早日期，换算为随访时间（年）。
# 步骤5：再按同样逻辑识别各癌症结局和全因死亡结局。
# 步骤6：输出的 event_* 与 time_* 是后续轨迹模型的核心输入，不能改变定义逻辑。
# ------------------------------------------------------------
# ============================================================
# ICD10-Code
# Author: JXW
