# ------------------------------------------------------------
# 【中文导读】本脚本进行多重插补（MICE），把协变量缺失补齐后用于后续模型。
# 步骤1：读取协变量并把分类变量统一转成factor，保证插补模型类型正确。
# 步骤2：执行5次插补并检查收敛（plot(imp)）。
# 步骤3：提取5套完整数据。
# 步骤4：连续变量按行取均值；分类变量按行取众数。
# 步骤5：与未插补变量合并并导出 covariates_impute.csv。
# ------------------------------------------------------------
# ============================================================
# Imputation
# Author: JXW
