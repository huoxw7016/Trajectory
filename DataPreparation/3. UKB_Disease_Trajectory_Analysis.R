# ------------------------------------------------------------
# 【中文导读】本脚本把疾病发生信息重构成多状态转移格式（trans1~trans9 + timefrom/timeto）。
# 步骤1：先合成CD事件（糖尿病或CVD），并计算CD首次时间。
# 步骤2：清除时间不合理记录（如负时间、同一时点冲突）。
# 步骤3：逐个个体按“是否发生CD/CA/死亡及先后顺序”进入不同if分支。
# 步骤4：在每个分支里写入对应转移指示与时间区间。
# 步骤5：该数据结构直接用于mstate/cox多状态建模。
# ------------------------------------------------------------
# ============================================================
